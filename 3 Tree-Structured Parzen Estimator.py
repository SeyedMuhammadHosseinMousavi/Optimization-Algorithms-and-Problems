...
def tpe_optimization(func, bounds, max_iter=200):
    ...
    trials = Trials()
    best = fmin(objective, space, algo=tpe.suggest, max_evals=max_iter, trials=trials, rstate=np.random.default_rng(42))
...
# Run TPE Optimization and display results for all functions
for idx, (name, func, bounds) in enumerate(functions):
    ...
    best_x, costs = tpe_optimization(func, bounds, max_iter=200)

    memory_after = memory_usage()[0]
    end_time = time.time()

    print(f"Function: {name}")
    print(f"Best Cost: {min(costs) if costs else 'N/A'}")
    print(f"Convergence Time: {end_time - start_time} seconds")
    print(f"Memory Usage: {max(0, memory_after - memory_before)} MB")
    print("Complexity Class: O(n)")
