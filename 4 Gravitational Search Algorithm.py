...
def gravitational_search_algorithm(func, bounds, population_size=50, generations=100, G0=100, alpha=20):
    ...
    masses = np.zeros(population_size)
    best_solution = None
    best_cost = float('inf')
    cost_history = []

    for generation in range(generations):
        # Calculate fitness and update masses
        ...
        masses = 1 / (fitness - np.min(fitness) + 1e-6)
        masses /= np.sum(masses)

        # Calculate gravitational constant
        G = G0 * np.exp(-alpha * generation / generations)
        ...
        for i in range(population_size):
            ...
            for j in range(population_size):
                if i != j:
                    ...
                    force += G * masses[i] * masses[j] * (population[j] - population[i]) / distance
...
# Map GSA Complexity to Standard Classes
def map_gsa_complexity(n, g, d):
    complexity_classes = {
        "O(1)": 1,
    ...
    time_complexity = n * d * g  # GSA complexity: population size × dimensions × generations
...
for idx, (name, func, bounds) in enumerate(functions):
    ...
    best_x, costs = gravitational_search_algorithm(func, bounds, population_size=population_size, generations=generations)

    memory_after = memory_usage()[0]
    end_time = time.time()

    # Map and report complexity
    complexity_category = map_gsa_complexity(population_size, generations, d)
