...
# Galaxy Gravity Optimization Algorithm
def galaxy_gravity_optimization(func, bounds, population_size=50, generations=100, G=1, mass_decay=0.98):
    ...
    masses = np.random.uniform(1, 5, population_size)  # Initialize comet masses
    best_solution = None
    best_cost = float('inf')
    cost_history = []
    ...
    for generation in range(generations):
        ...
        norm_fitness = (fitness - np.min(fitness)) + 1e-6
        masses = 1 / norm_fitness
        ...
        for i in range(population_size):
            ...
            for j in range(population_size):
                if i != j:
                    distance = np.linalg.norm(population[i] - population[j]) + 1e-6
                    gravitational_force = G * (masses[i] * masses[j]) / (distance**2)
                    force += gravitational_force * (population[j] - population[i])
            acceleration = force / masses[i]
            velocities[i] = velocities[i] * mass_decay + acceleration
...
# Map GGO Complexity to Standard Classes
def map_ggo_complexity(n, g, d):
    complexity_classes = {
        "O(1)": 1,
    ...
    time_complexity = n * d * g  # GGO complexity: population size × dimensions × generations
...
for idx, (name, func, bounds) in enumerate(functions):
    ...
    best_x, costs = galaxy_gravity_optimization(func, bounds, population_size=population_size, generations=generations)

    memory_after = memory_usage()[0]
    end_time = time.time()

    # Map and report complexity
    complexity_category = map_ggo_complexity(population_size, generations, d)
    ...
    print(f"Complexity (previous form): O(n * d * g)")
