...
def bbo_algorithm(func, bounds, population_size=50, generations=100, migration_rate=0.2, mutation_rate=0.01):
    dimensions = len(bounds)
    ...
    for generation in range(generations):
        ...
        for i in range(population_size):
            if np.random.rand() < migration_rate:
                donor_idx = np.random.randint(population_size)
                migration_vector = np.random.uniform(-1, 1, dimensions) * (population[donor_idx] - population[i])
                candidate = population[i] + migration_vector
        ...
        for i in range(population_size):
            if np.random.rand() < mutation_rate:
                mutation_vector = np.random.uniform(-1, 1, dimensions)
                candidate = population[i] + mutation_vector
                candidate = np.clip(candidate, [b[0] for b in bounds], [b[1] for b in bounds])
                candidate_cost = func(candidate)
                if candidate_cost < costs[i]:
                    population[i], costs[i] = candidate, candidate_cost
...
# Complexity Mapping Function (unchanged)
...
# Run BBO Algorithm and visualize results
...
for idx, (name, func, bounds) in enumerate(functions):
    ...
    best_x, costs = bbo_algorithm(func, bounds, population_size=50, generations=100)
