...
def ant_colony_optimization(func, bounds, num_ants=50, generations=100, evaporation_rate=0.5, alpha=1, beta=2):
    dimensions = len(bounds)
    pheromone = np.ones((num_ants, dimensions))
    best_cost = float('inf')
    best_solution = None
    costs = []
    ...
    for generation in range(generations):
        ...
        for ant in range(num_ants):
            solution = np.random.uniform([b[0] for b in bounds], [b[1] for b in bounds], dimensions)
            solutions.append(solution)

        scores = np.array([func(s) for s in solutions])
        best_idx = np.argmin(scores)
        if scores[best_idx] < best_cost:
...
# Run Ant Colony Optimization and plot results
...
for idx, (name, func, bounds) in enumerate(functions):
    ...
    best_x, costs = ant_colony_optimization(func, bounds, num_ants=50, generations=100)
