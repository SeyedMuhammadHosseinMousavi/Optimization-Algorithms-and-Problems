...
def best_first_search(func, x0, max_iter=200, step_size=0.1):
    x = np.array(x0)
    costs = [func(x)]

    for _ in range(max_iter):
        candidates = [x + np.random.uniform(-step_size, step_size, size=x.shape) for _ in range(10)]
        candidate_costs = [func(candidate) for candidate in candidates]
        best_candidate_idx = np.argmin(candidate_costs)

        if candidate_costs[best_candidate_idx] < costs[-1]:
            x = candidates[best_candidate_idx]
            costs.append(candidate_costs[best_candidate_idx])
        else:
...
# Run Best-First Search and display results for all functions
for idx, (name, func, x0) in enumerate(functions):
    ...
    best_x, costs = best_first_search(func, x0, max_iter=100, step_size=0.1)
