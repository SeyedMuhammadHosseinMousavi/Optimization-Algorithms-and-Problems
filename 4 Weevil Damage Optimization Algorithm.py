...
# WDOA Algorithm
def weevil_damage_optimization(func, bounds, population_size=50, generations=100, alpha=0.8, beta=0.2):
    ...
    for generation in range(generations):
        for i in range(population_size):
            # Damage and fly behaviors
            for j in range(population_size):
                if costs[j] < costs[i]:
                    # Simulate weevil movement and damage
                    fly = alpha * (population[j] - population[i])
                    damage = beta * np.random.uniform(-1, 1, dimensions)
                    population[i] = population[i] + fly + damage
...
# Run WDOA and visualize results
...
for idx, (name, func, bounds) in enumerate(functions):
    ...
    best_x, costs = weevil_damage_optimization(func, bounds, population_size=50, generations=100)
