...
def nelder_mead(func, x0, max_iter=200):
    costs = []

    def callback(xk):
        costs.append(func(xk))

    result = minimize(func, x0, method='Nelder-Mead', options={'maxiter': max_iter, 'disp': False}, callback=callback)
...
# Run Nelder-Mead and display results for all functions
for idx, (name, func, x0) in enumerate(functions):
    ...
    best_x, costs = nelder_mead(func, x0, max_iter=100)
