...
def evolution_strategy(func, bounds, population_size=50, generations=100, sigma=0.1):
    ...
    for generation in range(generations):
        ...
        for individual in population:
            noise = np.random.normal(0, sigma, dimensions)
            child = np.clip(individual + noise, [b[0] for b in bounds], [b[1] for b in bounds])
            offspring.append(child)

        population = sorted(population + offspring, key=func)[:population_size]
...
# Run Evolution Strategy and plot results
population_size = 50
generations = 100
...
for idx, (name, func, bounds) in enumerate(functions):
    ...
    best_x, costs = evolution_strategy(func, bounds, population_size=population_size, generations=generations)
