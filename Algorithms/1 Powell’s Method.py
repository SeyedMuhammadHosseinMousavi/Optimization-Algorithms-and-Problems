...
def powells_method(func, x0, max_iter=200):
    costs = []

    def callback(xk):
        costs.append(func(xk))

    result = minimize(func, x0, method='Powell', options={'maxiter': max_iter, 'disp': False}, callback=callback)
    return result.x, costs
...
# Run Powell's Method and display results for all functions
for idx, (name, func, x0) in enumerate(functions):
    ...
    best_x, costs = powells_method(func, x0, max_iter=100)
    ...
    print("Complexity Class: O(n^2 * d)")
