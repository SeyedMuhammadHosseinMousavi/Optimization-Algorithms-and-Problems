...
# VAO Algorithm
def victoria_amazonica_optimization(func, bounds, population_size=50, generations=100, mu=0.1, omega=0.9, psi=1.1):
    ...
    for _ in range(generations):
        # Intra-Competition and Mutation
        for i in range(population_size):
            for j in range(population_size):
                if costs[j] < costs[i]:
                    # Drawback (competition) and mutation
                    r = np.linalg.norm(population[i] - population[j])
                    population[i] = population[i] + mu * np.random.uniform(-1, 1, dimensions) - omega * (population[i] - population[j]) / (psi + r)
...
for idx, (name, func, bounds) in enumerate(functions):
    ...
    best_x, costs = victoria_amazonica_optimization(func, bounds, population_size=50, generations=100)
