...
# Gray Wolf Optimizer
def gray_wolf_optimizer(func, bounds, population_size=50, generations=100):
    dimensions = len(bounds)
    alpha, beta, delta = None, None, None
    alpha_score, beta_score, delta_score = float('inf'), float('inf'), float('inf')
    wolves = [np.random.uniform([b[0] for b in bounds], [b[1] for b in bounds], dimensions) for _ in range(population_size)]
    costs = []
    ...
    for generation in range(generations):
        ...
        for i in range(population_size):
            ...
            wolves[i] = np.clip((X1 + X2 + X3) / 3, [b[0] for b in bounds], [b[1] for b in bounds])

        costs.append(alpha_score)
...
# Run Gray Wolf Optimizer and plot results
...
for idx, (name, func, bounds) in enumerate(functions):
    ...
    best_x, costs = gray_wolf_optimizer(func, bounds, population_size=50, generations=100)
