...
def tlbo(func, bounds, population_size=50, generations=100):
    ...
    for generation in range(generations):
        ...
        teacher = population[np.argmin(costs)]
        mean_population = np.mean(population, axis=0)
        teaching_factor = np.random.randint(1, 3)
        for i in range(population_size):
            new_solution = population[i] + np.random.uniform(0, 1, dimensions) * (teacher - teaching_factor * mean_population)
        ...
        for i in range(population_size):
            ...
            if costs[j] < costs[i]:
                new_solution = population[i] + np.random.uniform(0, 1, dimensions) * (population[j] - population[i])
            else:
                new_solution = population[i] + np.random.uniform(0, 1, dimensions) * (population[i] - population[j])
...
# Run TLBO and visualize results
...
for idx, (name, func, bounds) in enumerate(functions):
    ...
    best_x, costs = tlbo(func, bounds, population_size=50, generations=100)
