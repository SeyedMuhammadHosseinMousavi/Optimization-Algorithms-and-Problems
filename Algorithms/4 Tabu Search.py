...
def tabu_search(func, bounds, max_iterations=100, tabu_size=5, neighborhood_size=10):
    dimensions = len(bounds)

    # Initialize solution
    current_solution = np.random.uniform([b[0] for b in bounds], [b[1] for b in bounds], dimensions)
    ...
    for iteration in range(max_iterations):
        ...
        for _ in range(neighborhood_size):
            neighbor = current_solution + np.random.uniform(-0.1, 0.1, dimensions)
            neighbor = np.clip(neighbor, [b[0] for b in bounds], [b[1] for b in bounds])
            neighbors.append(neighbor)

        # Evaluate neighbors
        neighbor_costs = [func(neighbor) for neighbor in neighbors]

        # Find the best neighbor not in tabu list
        for i, neighbor in enumerate(neighbors):
            if list(neighbor) not in tabu_list:
                if neighbor_costs[i] < current_cost:
                    current_solution = neighbor
                    current_cost = neighbor_costs[i]
                    break
...
def map_complexity_category(algorithm, n, max_iterations):
    """
    Maps the complexity to one of the eight standard categories.
    """
    # Tabu Search Complexity is O(n * max_iterations)
    complexity = n * max_iterations
...
# Run Tabu Search and visualize results
...
for idx, (name, func, bounds) in enumerate(functions):
    ...
    best_x, costs = tabu_search(func, bounds, max_iterations=100, tabu_size=5, neighborhood_size=10)
    ...
    complexity_category = map_complexity_category(algorithm, 10, 100)  # Example with neighborhood_size=10, max_iterations=100
    ...
    print(f"Complexity (previous form): O(n * max_iterations)")
