...
def hill_climbing(func, x0, max_iter=200, step_size=0.1):
    x = np.array(x0)
    costs = [func(x)]

    for _ in range(max_iter):
        candidate = x + np.random.uniform(-step_size, step_size, size=x.shape)
...
# Run Hill Climbing and display results for all functions
for idx, (name, func, x0) in enumerate(functions):
    ...
    best_x, costs = hill_climbing(func, x0, max_iter=100, step_size=0.1)
