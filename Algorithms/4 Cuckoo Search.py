...
def cuckoo_search(func, bounds, population_size=50, generations=100, pa=0.25, beta=1.5):
    dimensions = len(bounds)
    ...
    # Initialize nests and evaluate fitness
    nests = [np.random.uniform([b[0] for b in bounds], [b[1] for b in bounds], dimensions) for _ in range(population_size)]
    ...
    for generation in range(generations):
        ...
        for i in range(population_size):
            ...
            new_solution = nests[i] + step * (nests[np.random.randint(population_size)] - nests[i])
        ...
        # Abandon some nests and replace them
        abandon = np.random.rand(population_size) < pa
        for i in range(population_size):
            if abandon[i]:
                nests[i] = np.random.uniform([b[0] for b in bounds], [b[1] for b in bounds], dimensions)
...
# Run Cuckoo Search and visualize results
...
for idx, (name, func, bounds) in enumerate(functions):
    ...
    best_x, costs = cuckoo_search(func, bounds, population_size=50, generations=100)
