...
# Bat Algorithm
def bat_algorithm(func, bounds, population_size=50, generations=100, A=0.5, r=0.5, f_min=0, f_max=2):
    dimensions = len(bounds)

    # Initialize population
    bats = [np.random.uniform([b[0] for b in bounds], [b[1] for b in bounds], dimensions) for _ in range(population_size)]
    velocities = [np.zeros(dimensions) for _ in range(population_size)]
    fitness = [func(bat) for bat in bats]

    best_solution = bats[np.argmin(fitness)]
    best_cost = min(fitness)
    cost_history = []

    for generation in range(generations):
        # Adjust frequencies and velocities
        for i in range(population_size):
            frequency = f_min + (f_max - f_min) * np.random.rand()
            velocities[i] += (bats[i] - best_solution) * frequency
            new_bat = bats[i] + velocities[i]
            new_bat = np.clip(new_bat, [b[0] for b in bounds], [b[1] for b in bounds])

            # Pulse rate and loudness
            if np.random.rand() > r:
                new_bat = best_solution + 0.001 * np.random.randn(dimensions)
...
# Run Bat Algorithm and visualize results
...
for idx, (name, func, bounds) in enumerate(functions):
    ...
    best_x, costs = bat_algorithm(func, bounds, population_size=50, generations=100)
