...
def numerical_hessian(func, x, epsilon=1e-4):
    n = len(x)
    hessian = np.zeros((n, n))
    for i in range(n):
        for j in range(n):
            x_ij, x_i, x_j = x.copy(), x.copy(), x.copy()
            x_ij[i] += epsilon
            x_ij[j] += epsilon
            x_i[i] += epsilon
            x_j[j] += epsilon
            ...
            f_j = func(x_j)
            f = func(x)
            hessian[i, j] = (f_ij - f_i - f_j + f) / (epsilon**2)
    return hessian

def newton_method(func, x0, max_iter=200):
    ...
    for _ in range(max_iter):
        ...
        try:
            ...
            break  # If Hessian is singular, stop optimization
        x -= hessian_inv @ grad
        costs.append(func(x))
    return x, costs
...
# Run Newton Method and display results for all functions
for idx, (name, func, x0) in enumerate(functions):
    ...
    best_x, costs = newton_method(func, x0, max_iter=100)
