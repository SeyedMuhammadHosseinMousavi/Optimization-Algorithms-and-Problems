...
def whale_optimization_algorithm(func, bounds, population_size=50, generations=100):
    dimensions = len(bounds)

    # Initialize the population and fitness
    whales = [np.random.uniform([b[0] for b in bounds], [b[1] for b in bounds], dimensions) for _ in range(population_size)]
    fitness = [func(whale) for whale in whales]

    best_solution = whales[np.argmin(fitness)]
    best_cost = min(fitness)
    cost_history = []

    for generation in range(generations):
        for i in range(population_size):
            a = 2 - 2 * generation / generations  # Linearly decreasing parameter
            r = np.random.rand()  # Random number in [0, 1]

            if r < 0.5:
                p = np.random.rand()  # Probability to select shrinking encircling or spiral updating
                if p < 0.5:
                    # Shrinking encircling mechanism
                    A = a * (2 * np.random.rand(dimensions) - 1)
                    ...
                    new_position = D * np.exp(1j * l * np.pi).real + best_solution
            else:
                # Search for prey (random search)
            ...
            # Update the whale's position if the new position is better
...
# Run Whale Optimization Algorithm and visualize results
...
for idx, (name, func, bounds) in enumerate(functions):
    ...
    best_x, costs = whale_optimization_algorithm(func, bounds, population_size=50, generations=100)
